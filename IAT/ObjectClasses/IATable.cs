namespace IAT
{
    ...
    /// <summary>
    /// Contains data from a single table in an IAT file
    /// </summary>
	public class IATable
	{
        ...
        /// </summary>
		private List<string> row_names = new List<string>();			

        /// <summary>
        /// List of column names in the table
        /// </summary>
		private List<string> col_names = new List<string>();

        /// <summary>
        /// List containing secondary row data found in some tables
        /// </summary>
        private List<string> extra_names = new List<string>();			
		...
		public IATable(string name, IAT iat)
		{
            this.iat = iat;            
			this.name = name;
        ...
        private void LoadExtra()
        {
            ...
            foreach (Row row in table_rows)
            {
                ...
                extra_names.Add(iat.ParseCell(cell));
            }            
            return;
        }
        ...
        private T ConvertData<T>(string s)
        {
            ...
            {
                Toolbox.WriteError($"Table '{name}' contains an empty cell.", iat);
                return default(T);
            }
            catch (IndexOutOfRangeException)
            {
                Toolbox.WriteError($"Table '{name}' tried to access data outside the table.", iat);
                return default(T);
            }
            catch (InvalidCastException)
            {
                T t = default(T);
                Toolbox.WriteError($"Table '{name}' expected type {t.GetType().Name}, but did not find it.", iat);
                return default(T);
            }
            catch
            {
                Toolbox.WriteError($"Table '{name}' contains an unknown error.", iat);
                return default(T);
            }
        }
        ...
        /// <summary>
        /// Returns the name of the IATable.
        /// </summary>
        ...
        /// </summary>
        public List<string> GetRowNames() => row_names;

        /// <summary>
        /// Returns the list of column names. 
        /// </summary>
        public List<string> GetColNames() => col_names;

        /// <summary>
        /// Returns the list of extra row data.
        /// </summary>
        public List<string> GetExtra() => extra_names;
    }
}
