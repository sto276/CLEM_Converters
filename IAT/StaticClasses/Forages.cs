...
namespace IAT
{
    ...
    class Forages
    {
        ...
        {
            IATable fcg = iat.tables["Forage Crops Grown"];
            IATable fcs = iat.tables["Forage Crop Specifications"];
            IATable land = iat.tables["Land specifications"];            
            ...
            while (fcg.GetData<string>(0, col) != "0")
            {
                double area = fcg.GetData<double>(2, col);
                if (area > 0)
                {
                    int row = fcg.GetData<int>(0, col);
                    string name = fcs.GetRowNames()[row + 1];
                    ...
                    crop.Add(new XElement("LandItemNameToUse", $"Land.{land.GetRowNames()[num]}"));
                    crop.Add(new XElement("AreaRequested", fcg.GetData<string>(2, col)));
